...
On top of some useful defaults, seeding the database with ~1 million record across all tables is also supported. The default seeds are run automatically during the migration.

For the 1M record seed, it is generated by the `generate_seed.py` Python script in `src/main/resources/db/migration`. By default it will output 2 files: `V13__seed_1m_cases.sql` for SQL seeds, and `seed_mongodb.js` for NoSQL seeds.

> To reproduce UUIDs (useful when generating seed across a dedicated SQL server and a dedicated NoSQL server such that the NoSQL can correctly reference SQL data), make sure use the `SEED_VALUE` in `generate_seed.py` when generating the seed.

For the SQL seeds, simply run the Flyway migration. For the NoSQL seed, run
...
mongoimport --db court \
            ...
            --drop
mongoimport --db court \
            --collection docket_logs \
            --file docket_logs.json \
            --drop
```

Then, create indexes using:

```bash
mongosh "mongodb://court@localhost:27017/court" --eval "
...
> Be warned that the seeding of massive records might take a lot of RAM and can take long. Especially for the NoSQL seed.

If the 1M SQL seed is run, there will be the following default accounts:

| Username         | Email                         | Password    |
| ---------------- | ----------------------------- | ----------- |
| default.admin    | default.admin@court.gov.kh    | admin123    |
| default.chief    | default.chief@court.gov.kh    | chief123    |
| default.greffier | default.greffier@court.gov.kh | greffier123 |
